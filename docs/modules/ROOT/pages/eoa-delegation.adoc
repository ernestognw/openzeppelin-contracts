...
To remove the delegation and restore your EOA to its original state, you can send a `SET_CODE_TX_TYPE` transaction with an authorization tuple that points to the zero address (`0x0000000000000000000000000000000000000000`). This will clear the account's code and reset its code hash to the empty hash.
...
Similar to how xref:sending-user-operations.adoc#bundle_a_useroperation[sending a UserOp] is achieved for factory accounts, here's how you can construct a user op for an EOA who's delegated to an xref:api:account.adoc#Account[`Account`] contract.
